...
export function DateValidatorArr(fromControlToCompare: any): Array<ValidatorFn> {
  let ctrlName: string;
  ...
  if (fromControlToCompare == null) {
    ...
    ctrlName = 'end date';
    dateToCompare = fromControlToCompare.value;
  }
  return [
    f => !f.value ? { 'val': `'${ctrlName}'  is required` } : null,
    f => f.value && f.value < dateToCompare ? { 'val': `'${ctrlName}' must be above ${dateToCompare.toDateString()}` } : null
  ];
}